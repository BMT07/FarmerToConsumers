...
const { GetUserLivraison, PlaceOrder, Payment, Verify, updateOrder, GetOrder, cancelOrder, GetFarmerOrder, GetLivraisonByReference, Deliver } = require('../controllers/livraison.controller')
...
router.post('/AddBlog',async (req, res) => {
    try {
      const { Name, CompanyName, subject, description} = req.body;
      const blog = await Blog.create({
        Name, CompanyName, subject, description
      });
      res.json(blog);
    } catch (err) {
      console.error(err.message)
...
router.get('/searchProducts/:name',async(req,res)=>{
  try {
...
router.put('/Deliver/:order', passport.authenticate('jwt', { session: false }), Deliver)
